...
peaks <- toGRanges(paste0(DATA_DIR, 'Human.bed'), format="BED")
peaks[1:2]
...
write.table(anno_df, file=paste0(DATA_DIR, 'Human.genes.txt'),
            col.names = TRUE, row.names = FALSE, sep = '\t', quote = FALSE)

uniq_genes_df <- unique(anno_df['symbol'])
write.table(uniq_genes_df, file=paste0(DATA_DIR, 'Human.genes_uniq.txt'),
